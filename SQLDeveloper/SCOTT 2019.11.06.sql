...
SELECT ename, sal, NVL(comm,0) AS 커미션, NVL2(comm, sal+comm, sal) AS 총액
FROM emp
ORDER BY 4 DESC;
...
SELECT ename, sal, TO_CHAR(ROUND(sal*0.15,1), '$999.9') AS 회비
FROM emp
WHERE sal BETWEEN 1500 AND 3000;

--문제 36)사원수가 5명이 넘는 부서의 부서명과 사원수 조회

SELECT d.dname, COUNT(e.empno)  --기본키는 null값 혹은 중복이 될 수 없다.
FROM emp e
JOIN dept d
        ON e.deptno=d.deptno
GROUP BY d.dname
...
--문제 38)사원들의 사원번호(empno), 사원명(ename), 급여(sal), 급여등급(grade)을 출력하시오. 
--테이블 salgrade 조인
